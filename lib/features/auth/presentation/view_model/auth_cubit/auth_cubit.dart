...
class AuthCubit extends Cubit<AuthState> {
  ...
  Future<void> getRememberData() async {
    await authRepository.getRememberedData().then((value) => emit(
        state.copyWith(
            phone: value.dataResponse![AppString.userPhone],
            password: value.dataResponse![AppString.password],
            rememberMe: value.dataResponse![AppString.rememberMe])));
  }

  void rememberMeButtonOnChange(bool? rememberMe) {
  ...
  Future<void> register() async {
    ...
    ApiResult<void, Failure> result = await authRepository.register(
        registerEntity: state.registerEntity!
            .copyWith(phone: state.registerEntity!.phone!));
